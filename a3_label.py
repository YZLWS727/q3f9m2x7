...
TAGS_DEF = {
    ...
    "#期权": "所有与期权合约、期权市场直接相关的新闻，含ETF期权、股指/个股期权、商品期权、外汇期权等品种的行情、交易、持仓、到期、行权、隐含波动率、期权策略与监管；不泛指股票、期货、债券、ETF、加密货币等非期权行情，也不用于“股权激励/行权条款”等公司治理语境",
    "#期货": "所有与期货市场及品种相关的新闻",
    ...
    "#无法归类等待识别": "仅当所有其他标签都不符合时使用；此标签必须独占（唯一），禁止与其他任何标签同时出现",
}
